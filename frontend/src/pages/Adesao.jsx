...
import { contratosAdesao } from '../data/mockData';
import { Search, Filter, Plus, Eye, Edit, FileText, ChevronDown, Download } from 'lucide-react';
...
const Adesao = () => {
  ...
  const filtered = contratosAdesao.filter(c => {
    const matchSearch = c.numero.toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      c.administradora.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <div style={{ width: '36px', height: '36px', borderRadius: '8px', background: '#4979bb15', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <FileText size={18} color="#4979bb" />
          </div>
          <div>
            <h2 style={{ fontSize: '1.1rem', fontWeight: 600, color: '#344050', margin: 0 }}>Contratos por Adesão</h2>
            <p style={{ fontSize: '0.72rem', color: '#8a8d93', margin: 0 }}>Gerencie seus contratos de adesão</p>
          </div>
        </div>
        <Button
          style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            <div style={{ flex: 1, position: 'relative' }}>
              ...
              <Input
                placeholder="Buscar por número, seguradora, produto..."
                value={searchTerm}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px', fontSize: '0.72rem', color: '#8a8d93' }}>
        <span>Exibindo {filtered.length} de {contratosAdesao.length} contratos</span>
      ...
      <Dialog open={showDetail} onOpenChange={setShowDetail}>
        <DialogContent style={{ maxWidth: '600px' }}>
          <DialogHeader>
            <DialogTitle style={{ fontSize: '1rem' }}>Detalhes do Contrato</DialogTitle>
          </DialogHeader>
          {selectedContrato && (
            <div style={{ padding: '8px 0' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                ...
                <div>
                  <label style={{ fontSize: '0.68rem', color: '#8a8d93', textTransform: 'uppercase', fontWeight: 600 }}>Administradora</label>
                  <p style={{ fontSize: '0.85rem', color: '#344050', margin: '2px 0' }}>{selectedContrato.administradora}</p>
                </div>
                <div>
