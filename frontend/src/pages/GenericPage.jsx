...
import { Construction, User, Settings, HelpCircle, Mail, Save } from 'lucide-react';
      ...
      <div className="animate-fade-in" style={{ display: 'flex', justifyContent: 'center', padding: '24px' }}>
        <div style={cardStyle}>
          ...
          <div style={{ display: 'grid', gap: '12px' }}>
            ...
            <label><input type="checkbox" checked={settings.temaEscuro} onChange={(e) => setSettings((s) => ({ ...s, temaEscuro: e.target.checked }))} /> Ativar tema escuro</label>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: 'grid', gap: '10px' }}>
            <input
              ...
              placeholder="Assunto"
              style={inputStyle}
              value={support.assunto}
              onChange={(e) => setSupport((s) => ({ ...s, assunto: e.target.value }))}
            />
            <textarea
              aria-label="Mensagem"
              placeholder="Descreva sua solicitação"
              style={{ ...inputStyle, minHeight: '120px', resize: 'vertical' }}
              value={support.mensagem}
              onChange={(e) => setSupport((s) => ({ ...s, mensagem: e.target.value }))}
            />
          </div>
          <div style={{ marginTop: '14px', display: 'flex', gap: '10px', alignItems: 'center' }}>
            <a href={mailToHref} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '8px 14px', borderRadius: '8px', background: '#4979bb', color: '#fff', textDecoration: 'none', fontWeight: 600 }}>
              <Mail size={16} /> Enviar e-mail para {supportEmail}
            </a>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 'calc(100vh - 200px)' }}>
      <div style={{
        ...
        <div style={{ width: '64px', height: '64px', borderRadius: '16px', background: '#4979bb15', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px' }}>
