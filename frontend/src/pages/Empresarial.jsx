    ...
    <div className="animate-fade-in">
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'16px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
          ...
          <div><h2 style={{ fontSize:'1.1rem', fontWeight:600, color:'#344050', margin:0 }}>Contratos Empresariais / PME</h2><p style={{ fontSize:'0.72rem', color:'#8a8d93', margin:0 }}>Gerencie seus contratos empresariais</p></div>
        </div>
      ...
      <Dialog open={showDetail} onOpenChange={setShowDetail}><DialogContent style={{maxWidth:'650px'}}><DialogHeader><DialogTitle>Detalhes do Contrato Empresarial</DialogTitle></DialogHeader>
        {selectedContrato && (<div style={{padding:'8px 0'}}><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}}>
          {[['Número',selectedContrato.numero,'#2C7BE5'],['Status',null],['Empresa',selectedContrato.empresa],['CNPJ',selectedContrato.cnpj],['Seguradora',selectedContrato.seguradora],['Produto',selectedContrato.produto],['Vigência',selectedContrato.vigencia],['Vencimento',selectedContrato.vencimento],['Vidas',selectedContrato.vidas],['Valor Mensal',selectedContrato.valorMensal,'#27ae60']].map(([lbl,val,clr],i)=>(
      ...
      <Dialog open={showNew} onOpenChange={setShowNew}><DialogContent style={{maxWidth:'600px'}}><DialogHeader><DialogTitle>Novo Contrato Empresarial</DialogTitle></DialogHeader>
        <div style={{display:'flex',flexDirection:'column',gap:'10px',padding:'8px 0'}}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
            <div><label style={{fontSize:'0.72rem',color:'#8a8d93',fontWeight:600}}>Número</label><Input value={formData.numero} onChange={e=>setFormData({...formData,numero:e.target.value})} placeholder="EMP-2024-XXX"/></div>
            <div><label style={{fontSize:'0.72rem',color:'#8a8d93',fontWeight:600}}>Empresa</label><Input value={formData.empresa} onChange={e=>setFormData({...formData,empresa:e.target.value})}/></div>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
            <div><label style={{fontSize:'0.72rem',color:'#8a8d93',fontWeight:600}}>CNPJ</label><Input value={formData.cnpj} onChange={e=>setFormData({...formData,cnpj:e.target.value})} placeholder="00.000.000/0000-00"/></div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'10px'}}>
            ...
            <div><label style={{fontSize:'0.72rem',color:'#8a8d93',fontWeight:600}}>Vencimento</label><Input value={formData.vencimento} onChange={e=>setFormData({...formData,vencimento:e.target.value})} placeholder="01/01/2025"/></div>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
