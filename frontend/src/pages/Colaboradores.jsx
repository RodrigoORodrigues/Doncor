...
const Colaboradores = () => {
  ...
  const [form, setForm] = useState({ nome:'', cargo:'', email:'', telefone:'', departamento:'', status:'Ativo' });
  const [saving, setSaving] = useState(false);

  const loadData = useCallback(async () => {
    setLoading(true);
    try { setData(await fetchColaboradores(search)); } catch(e){console.error(e);}
    setLoading(false);
  }, [search]);
  ...
  const handleCreate = async () => {
    setSaving(true);
    try { await createColaborador(form); setShowNew(false); setForm({ nome:'', cargo:'', email:'', telefone:'', departamento:'', status:'Ativo' }); loadData(); } catch(e){console.error(e);}
    setSaving(false);
  };

  const handleSaveEdit = async (id) => {
    setSaving(true);
    try { await updateColaborador(id, editData); setEditingId(null); loadData(); } catch(e){console.error(e);}
    setSaving(false);
  };

  const handleDelete = async (id) => {
    if (!window.confirm('Excluir colaborador?')) return;
    try { await deleteColaborador(id); loadData(); } catch(e){console.error(e);}
  };
  ...
  return (
    <div className="animate-fade-in">
      ...
      {showFilters && (<div style={{ background:'#fff', borderRadius:'0 0 8px 8px', padding:'16px', marginBottom:'12px', boxShadow:'0 1px 3px rgba(0,0,0,0.06)' }}>
        <div style={{flex:1,position:'relative'}}><Search size={14} style={{position:'absolute',left:'10px',top:'50%',transform:'translateY(-50%)',color:'#8a8d93'}}/><Input placeholder="Buscar nome, cargo, email..." value={search} onChange={e=>{setSearch(e.target.value);setPage(1);}} style={{paddingLeft:'32px',fontSize:'0.8rem'}}/></div>
      </div>)}
      ...
      <div style={{ background:'#fff', borderRadius:'8px', boxShadow:'0 1px 3px rgba(0,0,0,0.06)', overflow:'hidden', marginTop:showFilters?'0':'12px' }}>
        {loading ? <div style={{display:'flex',justifyContent:'center',padding:'40px'}}><Loader2 size={24} style={{color:'#2C7BE5',animation:'spin 1s linear infinite'}}/></div> : (
          <table className="data-table"><thead><tr><th>Nome</th><th>Cargo</th><th>Email</th><th>Telefone</th><th>Departamento</th><th>Admissão</th><th>Status</th><th style={{width:'100px'}}>Ações</th></tr></thead>
            <tbody>{paged.map(item=>(<tr key={item.id}>
              {editingId===item.id ? (<>
                <td>{inlineInput('nome')}</td><td>{inlineInput('cargo')}</td><td>{inlineInput('email')}</td><td>{inlineInput('telefone')}</td>
                <td><select value={editData.departamento} onChange={e=>setEditData({...editData,departamento:e.target.value})} style={{fontSize:'0.75rem',padding:'4px 6px',border:'1px solid #d8e2ef',borderRadius:'4px',width:'100%'}}><option>Diretoria</option><option>Comercial</option><option>Operações</option><option>Financeiro</option></select></td>
                ...
                <td style={{fontWeight:600}}>{item.nome}</td><td>{item.cargo}</td><td style={{fontSize:'0.75rem',color:'#2C7BE5'}}>{item.email}</td><td style={{fontSize:'0.75rem'}}>{item.telefone}</td>
                <td><span style={{background:'#edf2f9',padding:'2px 8px',borderRadius:'10px',fontSize:'0.68rem',fontWeight:500}}>{item.departamento}</span></td>
      ...
      <Dialog open={showNew} onOpenChange={setShowNew}><DialogContent style={{maxWidth:'550px'}}><DialogHeader><DialogTitle>Novo Colaborador</DialogTitle></DialogHeader>
        <div style={{display:'flex',flexDirection:'column',gap:'10px',padding:'8px 0'}}>
          <div><label style={{fontSize:'0.72rem',color:'#8a8d93',fontWeight:600}}>Nome Completo</label><Input value={form.nome} onChange={e=>setForm({...form,nome:e.target.value})}/></div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
            <div><label style={{fontSize:'0.72rem',color:'#8a8d93',fontWeight:600}}>Cargo</label><Input value={form.cargo} onChange={e=>setForm({...form,cargo:e.target.value})}/></div>
            <div><label style={{fontSize:'0.72rem',color:'#8a8d93',fontWeight:600}}>Departamento</label><select value={form.departamento} onChange={e=>setForm({...form,departamento:e.target.value})} style={{width:'100%',border:'1px solid #d8e2ef',borderRadius:'6px',padding:'8px 12px',fontSize:'0.85rem'}}><option value="">Selecione</option><option>Diretoria</option><option>Comercial</option><option>Operações</option><option>Financeiro</option></select></div>
          </div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
        ...
        <DialogFooter><Button variant="outline" onClick={()=>setShowNew(false)}>Cancelar</Button><Button style={{background:'#2C7BE5',color:'#fff'}} onClick={handleCreate} disabled={saving}>{saving?'Salvando...':'Criar'}</Button></DialogFooter>
