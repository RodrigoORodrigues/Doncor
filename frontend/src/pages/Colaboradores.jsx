...
const Colaboradores = ({ session, accessByRole, onAccessChange }) => {
  ...
  const handleCreate = async () => {
    setSaving(true);
    try { await createColaborador(form); setShowNew(false); setForm({ nome:'', cargo:'', email:'', telefone:'', departamento:'Diretoria', status:'Ativo', usuario:'', senha:'' }); loadData(); } catch(e){console.error(e);} setSaving(false);
  };
          ...
          <div style={{display:'flex',flexDirection:'column',gap:'10px',padding:'8px 0'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
              ...
              <div>
                ...
                <select value={form.departamento} onChange={e=>setForm({...form,departamento:e.target.value})} style={{width:'100%',border:'1px solid #d8e2ef',borderRadius:'6px',padding:'8px 12px',fontSize:'0.85rem'}}>{FIXED_ROLES.map((r)=><option key={r}>{r}</option>)}</select>
              </div>
            </div>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
              <div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px'}}>
              <div>
                ...
                <Input value={form.usuario} onChange={e=>setForm({...form,usuario:e.target.value})} placeholder="Ex: analista.suporte"/>
              </div>
              <div>
                <label style={{fontSize:'0.72rem',color:'#8a8d93',fontWeight:600}}>Senha</label>
                <Input value={form.senha} onChange={e=>setForm({...form,senha:e.target.value})} placeholder="Digite a senha de acesso"/>
              </div>
            </div>
          </div>
          <DialogFooter>
